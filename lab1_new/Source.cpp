...
int WINAPI _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdShow, int nCmdShow)
{
	...
	if (!RegisterClass(&wc))
	{
		MessageBox(NULL, _T("Класс не зарегистрирован"), NULL, MB_OK);
		return 0;
	}
	...
	if (!hWnd)
	{
		MessageBox(NULL, _T("Окно не создано"), NULL, MB_OK);
		return 0;
	}
	else
	{
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	HDC hdc;
	PAINTSTRUCT ps; 
	static HRGN hRgn; //регион для рисования
	...
	const int step = 20; //шаг откусывания
	...
	case WM_SIZE:
		...
		hRgn = CreateEllipticRgn(main_circle.left, main_circle.top, main_circle.right, main_circle.bottom);
